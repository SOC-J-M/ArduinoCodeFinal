...
unsigned long pushTime =  20000;         // push a record every 10 min
...
void setup() {
  Serial.begin(9600);
  Serial1.begin(9600);
  pinMode(counterPin, INPUT_PULLUP);             // set counter pin
...
void validateScan() {
  ...
  if (isDigit(scanned.charAt(0)) && !scanned.equals(workOrder)) { // check if new work order
    ...
    if (scanned.substring(6) == "RAW MATERIAL BAD") {
      statusCode = "RAW_MATERIAL_BAD";
    } else if (scanned.substring(6) == "RAW MATERIAL NOT AVAILABLE") {
      statusCode = "RAW_MATERIAL_NOT_AVAILABLE";
    } else if (scanned.substring(6) == "TOOLING NOT AVAILABLE") {
      statusCode = "TOOLING_NOT_AVAILABLE";
    } else {
      statusCode = scanned.substring(6);
    }
  }
...
void updateLCD() {
  ...
  if (statusCode == "RAW_MATERIAL_BAD") {
    lcd.print("RAW MATERIAL");
    lcd.setCursor(0, 3);
    lcd.print("BAD");
  } else if (statusCode == "RAW_MATERIAL_NOT_AVAILABLE") {
    lcd.print("RAW MATERIAL");
    lcd.setCursor(0, 3);
    lcd.print("NOT AVAILABLE");
  } else if (statusCode == "TOOLING_NOT_AVAILABLE") {
    lcd.print("TOOLING");
    lcd.setCursor(0, 3);
    lcd.print("NOT AVAILABLE");
  } else {
